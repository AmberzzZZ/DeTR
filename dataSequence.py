...
class dataSequence(Sequence):
    ...
    def __init__(self, img_dir, anno_dir, input_shape, n_classes, batch_size=2, max_boxes=100,
        ...
        assert max_size<=max(input_shape)
    ...
    def batch_data_generator_v1(self, batch_indices):
        ...
        for sample_idx in batch_indices:
            ...
            img, boxes = self.data_generator(sample_idx, self.is_train, self.max_size)
    ...
    def data_generator(self, index, is_train, max_size=1333):
        ...
        if is_train:
            ...
            if random.uniform(0, 1)>0.5:
                ...
                img, boxes = resize(img, boxes, size, max_size=self.max_size)
...
def resize(img, boxes, size, max_size):
    ...
    if size*ratio>max_size:
        size = int(max_size / ratio)
    if w > h:
        new_w, new_h = (int(size*ratio), size)
    else:
